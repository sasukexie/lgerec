...
class LLMKG(KnowledgeRecommender):
    ...
    def __init__(self, config, dataset):
        super(LLMKG, self).__init__(config, dataset)
        ...
        self.tanh = nn.Tanh() # GELU,ReLU,swish,Tanh(),Sigmoid()
        ...
        # method
        self.data_aug = config['data_aug'] #   # sen
        self.noise_base = config["noise_base"]

        # gen random noise
        self.temperature = config['temperature']
        self.n_layers_q = config['n_layers_q']
        self.n_layers_k = config['n_layers_k']

        self.ce_loss = nn.CrossEntropyLoss()  # ce
    ...
    def init_graph(self):
        ...
        for rel_type in range(1, self.n_relations, 1):
            ...
            sub_graph = (
                dgl.edge_subgraph(self.ckg, edge_idxs, relabel_nodes=False)  # , preserve_nodes=True 不能用
                .adj_external(scipy_fmt='coo', transpose=False)  # transpose=False, scipy_fmt="coo" #参考：https://docs.dgl.ai/generated/dgl.DGLGraph.adj.html#dgl.DGLGraph.adj
                .astype("float")
            )
            row_sum = np.array(sub_graph.sum(1))
    ...
    def calculate_loss(self, interaction):
        ...
        loss += self.cl_loss(interaction, self.batch_idx, user, pos_item, u_embeddings, pos_i_embeddings)
    ...
    def cl_loss(self, interaction, batch_idx, user, pos_item, u_embeddings, pos_i_embeddings):
        ...
        if str.startswith(self.data_aug, 'sen'):
            ...
            # data aug
            try:
                if self.data_aug == 'sen_a':
                    gaussian_noise = tool.add_gaussian_noise
                elif self.data_aug == 'sen_m':
                    gaussian_noise = tool.mul_gaussian_noise
                u_embedding_1 = gaussian_noise(u_embedding_1, self.noise_base).detach()
                u_embedding_2 = gaussian_noise(u_embedding_2, self.noise_base).detach()
                e_embedding_1 = gaussian_noise(e_embedding_1, self.noise_base).detach()
                e_embedding_2 = gaussian_noise(e_embedding_2, self.noise_base).detach()
        ...
        u_loss = self.loss1(u_embedding_1, u_embedding_2, batch_size=u_embedding_1.shape[0])
        e_loss = self.loss1(e_embedding_1, e_embedding_2, batch_size=e_embedding_1.shape[0])
        ui_loss = self.loss1(u_embeddings, pos_i_embeddings, batch_size=u_embeddings.shape[0])
        loss += 0.01 * (u_loss + e_loss + ui_loss)

        return loss

    def loss1(self, z_i, z_j, batch_size):  # B * D    B * D
        N = 2 * batch_size

        z = torch.cat((z_i, z_j), dim=0)  # 2B * D  ,z*z转置
