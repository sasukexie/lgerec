...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        saved_model_file = "../checkpoint/{}-{}-{}.pth".format(self.config["model"], self.config["dataset"], get_local_time())
        self.saved_model_file = os.path.join(self.checkpoint_dir, saved_model_file)
        ensure_dir(os.path.dirname(self.saved_model_file))
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        if hasattr(self.model, 'train_epoch'):
            self.model.train_epoch(train_data,epoch_idx)
        ...
        for batch_idx, interaction in enumerate(iter_data):
            self.model.batch_idx = batch_idx
    ...
    def evaluate(
        ...
        #     checkpoint_file = './saved/checkpoint/LGERec-lfm1b-tracks-ok.pth'
...
class LLMKGTrainer(Trainer):
    r"""KGATTrainer is designed for KGAT, which is a knowledge-aware recommendation method."""

    def __init__(self, config, model):
        super(LLMKGTrainer, self).__init__(config, model)
